...
exports.handle_database = function(req, res, sql) {
	db.query(sql, function(err, rows) {
		if (!err) {
			...
			res.json({"code" : 100, "status" : "Error with database"});
		}
	});
}
...
exports.insertTeam = function(id_equipo) { 
	...
	var request = http.get(url, function (response) {
    	...
    	response.on("end", function (err) {
			...
			db.statement("REPLACE INTO team set ? ", team, function(err, rows){
				if (err) {
...
var data = pelotero.data.rows[0];
	...
	db.statement("INSERT INTO player set ? ", player, function(err, rows){
		if (err) {
			console.log("Error inserting : %s ", err);
